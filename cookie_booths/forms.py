...
class BoothLocationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            self.cleaned_data["booth_block_level_restrictions_start"]
            > self.cleaned_data["booth_block_level_restrictions_end"]
            != 0
        ):
            self.add_error(
                "booth_block_level_restrictions_start",
                "Restriction start must be lower than end",
            )
            self.add_error(
                "booth_block_level_restrictions_end",
                "Restriction start must be lower than end",
...
class BoothHoursForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "booth_start_date": DatePickerInput(format="%m/%d/%Y").start_of(
                "booth days"
            ),
            "booth_end_date": DatePickerInput(format="%m/%d/%Y").end_of("booth days"),
            "sunday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "sunday_time"}
            ).start_of("sunday"),
            "sunday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "sunday_time"}
            ).end_of("sunday"),
            "monday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "monday_time"}
            ).start_of("monday"),
            "monday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "monday_time"}
            ).end_of("monday"),
            "tuesday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "tuesday_time"}
            ).start_of("tuesday"),
            "tuesday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "tuesday_time"}
            ).end_of("tuesday"),
            "wednesday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "wednesday_time"}
            ).start_of("wednesday"),
            "wednesday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "wednesday_time"}
            ).end_of("wednesday"),
            "thursday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "thursday_time"}
            ).start_of("thursday"),
            "thursday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "thursday_time"}
            ).end_of("thursday"),
            "friday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "friday_time"}
            ).start_of("friday"),
            "friday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "friday_time"}
            ).end_of("friday"),
            "saturday_open_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "saturday_time"}
            ).start_of("saturday"),
            "saturday_close_time": TimePickerInput(
                format="%I:%M %p", attrs={"id": "saturday_time"}
            ).end_of("saturday"),
        }
    ...
    def __check_times_set_correctly(self, day_of_week):
        ...
        if (self.cleaned_data[day_of_week_checkbox]) and (
            self.cleaned_data[day_of_week_open] is None
            or self.cleaned_data[day_of_week_close] is None
        ):
            if self.cleaned_data[day_of_week_open] is None:
                self.add_error(
                    day_of_week_open,
                    f"Please specify valid open time for {str.title(day_of_week)}.",
                )
            if self.cleaned_data[day_of_week_close] is None:
                self.add_error(
                    day_of_week_close,
                    f"Please specify valid close time for {str.title(day_of_week)}.",
                )
